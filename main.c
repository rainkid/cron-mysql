...
static void Cron_File_Config(const char *taskConfigFile);
static void Cron_Mysql_Config();
...
static void Cron_Mysql_Config() {
	...
	if (mysql_real_connect(&mysql_conn, host, user, passwd, db, 3306, NULL, 128) == NULL) {
		fprintf(stderr, "%s\n", "Connection fails.");
		mysql_close(&mysql_conn);
		return;
	}
